			   ...
			   GError **error)
{
	...
	description = fu_struct_asus_hid_description_parse_stream(stream, FGA_OFFSET, error);
	if (description == NULL)
		return FALSE;
	self->fga = g_strdup(fu_struct_asus_hid_description_get_fga(description));
	self->product = g_strdup(fu_struct_asus_hid_description_get_product(description));
	self->version = g_strdup(fu_struct_asus_hid_description_get_version(description));
	return TRUE;
}
