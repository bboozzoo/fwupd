			    ...
			    const gchar *path)
{
	...
	ret = fu_efivars_set_data_bytes(efivars, guid, name, mock_bytes, 0, &error);
	g_assert_no_error(error);
	g_assert_true(ret);
}

static void
fu_test_mock_dbx_efivars(FuEfivars *efivars)
{
	...
	fu_test_mock_efivar_content(efivars, FU_EFIVARS_GUID_EFI_GLOBAL, "KEK", mock_kek_path);
	fu_test_mock_efivar_content(efivars,
...
fu_uefi_dbx_test_plugin_update(FuTestFixture *fixture, gconstpointer user_data)
{
	...
	fu_test_mock_dbx_efivars(efivars);
